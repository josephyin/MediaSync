...
> 当前正式版本为 `v0.2.0`。普通用户可以用一个容器直接运行，并可在系统设置中使用实验性一键更新；API、Scheduler、Worker 和 Nginx 在容器内仍是职责独立的进程。阿里云盘、夸克网盘、123 云盘和百度网盘 Web 私有接口可能随上游更新或账号风控失效。
...
- 私有接口模式支持在 MediaSync 本机生成阿里云盘、夸克网盘、123 云盘和百度网盘登录二维码；扫码确认后凭证由后端直接加密保存，不经过 MediaSync 官方取 token 服务。
...
OpenAPI 授权方式按网盘的真实续期能力提供，不强行做成相同选项：

| 网盘 | AListGo | OpenList APIPages | 自有开放平台应用 |
|---|---|---|---|
| 阿里云盘 | 支持 | 支持 | 支持 |
...
自有应用模式由 MediaSync 直接请求官方 OAuth/OpenAPI。扫码私有登录仍负责分享读取和
转存，OpenAPI 只承担各网盘页面注明的账号盘能力。
...
- [x] 发布 `v0.2.0-rc.14` NAS 容器识别修复候选版
- [x] 发布 `v0.2.0-rc.17` NAS 设备映射继承修复候选版
- [x] 完成 v0.2 稳定性观察（维护者确认通过）
- [x] 发布 `v0.2.0-rc.18` 夸克网盘实验性候选版
- [x] 发布 `v0.2.0-rc.19` 自动回滚原因可见性修复候选版
- [x] 发布 `v0.2.0-rc.20` NAS 受限数据目录快照修复候选版
- [x] 发布 `v0.2.0-rc.21` Docker updater 身份字段规范化修复候选版
- [x] 发布 `v0.2.0-rc.22` NAS canonical capability 修复候选版
