...
> 当前候选版本为 `v0.2.0-rc.12`。普通用户可以用一个容器直接运行，并可在系统设置中使用实验性一键更新；API、Scheduler、Worker 和 Nginx 在容器内仍是职责独立的进程。版本仍处于稳定性观察期，默认 Web 私有接口可能随上游更新失效。
...
docker run -d \
  ...
  --stop-timeout 120 \
  josephyjq/mediasync:v0.2.0-rc.12
```
...
MEDIASYNC_IMAGE_TAG=v0.2.0-rc.12
```
...
- [x] 发布 `v0.2.0-rc.12` Web 会话过期跳转修复候选版
- [ ] 完成 v0.2 八周稳定性观察
- [ ] 发布 `v0.2.0` 正式版
- [ ] 夸克网盘 Provider
- [ ] 115 Provider
