...
> 当前候选版本为 `v0.2.0-rc.9`。普通用户可以用一个容器直接运行；API、Scheduler、Worker 和 Nginx 在容器内仍是职责独立的进程。版本仍处于稳定性观察期，默认 Web 私有接口可能随上游更新失效。
...
docker run -d \
  ...
  --stop-timeout 120 \
  josephyjq/mediasync:v0.2.0-rc.9
```
...
MEDIASYNC_IMAGE_TAG=v0.2.0-rc.9
```
