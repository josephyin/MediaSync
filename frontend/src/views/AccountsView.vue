...
const openForm = reactive({
  ...
  refresh_token: '',
  token_url: hostedTokenUrls.alistgo as string,
  client_id: '',
  client_secret: '',
})
...
async function load() {
  ...
  try {
    accounts.value = (await api<Page<CloudAccount>>('/cloud-accounts?page_size=100')).items
  } catch (error) {
...
function openAdd() {
  ...
  Object.assign(form, { provider: 'aliyundrive', name: '', refresh_token: '' })
  dialog.value = true
}
...
function openOpenApi(account: CloudAccount) {
  ...
  Object.assign(openForm, {
    mode: account.open_auth_mode ?? 'alistgo',
    refresh_token: '',
    token_url: account.open_token_url ?? hostedTokenUrls.alistgo,
...
function openModeChanged(mode: 'alistgo' | 'openlist' | 'custom') {
  if (mode !== 'custom') openForm.token_url = hostedTokenUrls[mode]
}
  ...
  <section>
    <div class="page-heading">
      ...
      <div class="page-actions">
        ...
        <el-button type="primary" @click="openQr()"><AppIcon name="cloud" />扫码添加</el-button>
      </div>
    </div>
    ...
    <div v-loading="pageLoading" class="account-grid">
      <article v-for="account in accounts" :key="account.id" class="account-card">
        <header class="account-card__header">
          <div class="account-brand"><span class="account-logo">Ali</span><div><h3>{{ account.name }}</h3><p>{{ account.account_identity || '账号信息待校验' }}</p></div></div>
          <el-tag :type="statusType(account.status)" effect="light">{{ statusLabel(account.status) }}</el-tag>
        </header>

        <div class="account-facts">
          <div><span>服务商</span><strong>Aliyun Drive</strong></div>
        ...
        <div class="open-panel" :class="{ connected: account.open_auth_mode }">
          <div class="open-panel__title">
            <div><span class="open-mark">O</span><div><strong>Aliyun Drive OpenAPI</strong><p>{{ account.open_auth_mode ? `${openModeLabels[account.open_auth_mode]} 授权` : '可选，用于识别默认盘、资源库与备份盘' }}</p></div></div>
        ...
        <footer class="account-actions">
          <el-button type="primary" plain :loading="verifyingId === account.id" @click="verify(account.id)">校验私有接口</el-button>
          <el-button @click="openEdit(account)">编辑账号</el-button>
          <el-dropdown trigger="click">
            <el-button>更多<AppIcon name="arrow" /></el-button>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item @click="openQr(account)">重新扫码登录</el-dropdown-item>
                <el-dropdown-item @click="openOpenApi(account)">{{ account.open_auth_mode ? '编辑 OpenAPI' : '绑定 OpenAPI' }}</el-dropdown-item>
                <el-dropdown-item v-if="account.open_auth_mode" :disabled="openVerifyingId === account.id" @click="verifyOpenApi(account)">校验 OpenAPI</el-dropdown-item>
      ...
      <div v-if="!pageLoading && !accounts.length" class="empty-state account-empty">
        ...
        <el-button type="primary" @click="openQr()">扫码添加第一个账号</el-button>
      </div>
    </div>

    <el-dialog v-model="dialog" :title="editingId ? '编辑云盘账号' : '手动添加账号'" width="min(480px, calc(100vw - 32px))">
      <el-form label-position="top">
        <el-form-item label="Provider"><el-select v-model="form.provider" class="full-width" :disabled="!!editingId"><el-option label="Aliyun Drive" value="aliyundrive" /></el-select></el-form-item>
        <el-form-item label="账号名称"><el-input v-model="form.name" placeholder="例如：家庭影音盘" /></el-form-item>
        <el-form-item :label="editingId ? 'Refresh Token（留空表示不修改）' : 'Refresh Token'"><el-input v-model="form.refresh_token" type="password" show-password /></el-form-item>
      </el-form>
    ...
    <el-dialog v-model="openDialog" title="绑定 Aliyun Drive OpenAPI" width="min(560px, calc(100vw - 32px))">
      <el-alert title="私有 token 继续负责分享监控和转存；Open token 只用于识别默认盘、资源库和备份盘。校验时会核对两边是否为同一账号。" type="info" :closable="false" class="open-alert" />
      <el-form label-position="top">
        ...
        <el-alert v-if="openForm.mode === 'openlist'" type="info" :closable="false" class="open-alert">
          <template #title>请在 OpenList APIPages 选择“阿里云盘 (OAuth2) 扫码登录”获取专用 token，并在下方选择同一节点：<el-link href="https://api.oplist.org.cn" target="_blank" type="primary">国内站</el-link> · <el-link href="https://api.oplist.org" target="_blank" type="primary">全球站</el-link></template>
        </el-alert>
        ...
        <el-form-item v-else-if="openForm.mode === 'openlist'" label="OpenList 刷新节点">
          ...
          <div class="form-tip">自建 APIPages 可以直接粘贴完整的 HTTPS /alicloud/renewapi 地址。</div>
        </el-form-item>
        <template v-else>
          <el-form-item label="Client ID"><el-input v-model="openForm.client_id" /></el-form-item>
          <el-form-item label="Client Secret（编辑时留空表示不修改）"><el-input v-model="openForm.client_secret" type="password" show-password /></el-form-item>
        </template>
