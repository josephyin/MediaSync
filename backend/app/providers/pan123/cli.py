...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a non-persistent, read-only 123 Cloud Drive capability probe. "
...
def _prompt_share_details(*, token_from_stdin: bool) -> tuple[str, str]:
    if not token_from_stdin:
        return (
            input("123 share URL: ").strip(),
            getpass.getpass("Share password (hidden, optional): "),
        )
    try:
        with open("/dev/tty", "r+", encoding="utf-8") as terminal:
            terminal.write("123 share URL: ")
            terminal.flush()
        ...
        raise ValueError(
            "An interactive terminal is required when checking a share with --token-stdin"
        ) from exc
...
async def _run(args: argparse.Namespace) -> dict[str, object]:
    ...
    async with Pan123ReadOnlyProbe(token, timeout_seconds=args.timeout) as probe:
...
def main() -> None:
    ...
    try:
        ...
        print("诊断已取消；Token 未保存。", file=sys.stderr)
        raise SystemExit(130) from None
    except (Pan123ProbeError, ValueError) as exc:
        result = {
            "schema_version": 1,
            "provider": "pan123",
            "mode": "readonly_private_api",
            "persisted": False,
            "error": {
                "code": getattr(exc, "code", "PAN123_INPUT_INVALID"),
                "message": str(exc),
            },
        }
