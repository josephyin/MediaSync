...
逐项保存分享子目录文件时被分享转存接口以业务码 `101304` 拒绝。此前成功的写入
探针只覆盖分享根层单项，不能证明该接口支持对子目录叶子文件逐项保存。

### 修复

- 123 文件任务在确认源文件仍属于目标分享后，改用上游 `Etag + Size` 的同盘内容
  复用请求保存到已经准备好的目标目录，不再把子目录叶子文件提交给仅验证过根层
  的分享转存请求。
- 只有上游明确返回 `Reuse=true` 才接受写入；随后继续沿用可恢复操作标识和目标
  目录对账，避免把上传会话或不完整响应误报为转存成功。
- 文件夹写入探针仍保留原分享转存契约，实际订阅扫描只为叶子文件创建转存任务。

### 验证

- 新增“分享子目录文件 -> 非根目标目录”的请求契约和目标端二次核验回归测试。
- 新增内容无法复用与业务码 `101304` 的明确写入失败分类测试。
- 完整后端测试 702 项、前端测试 5 项、Ruff、前端生产构建和全新数据库迁移往返
  验证通过。
...
沿用现有 Provider 契约接入账号、分享扫描、目标目录和转存任务链路。

### 新增

- 新增 123 云盘账号、分享分页、目标目录解析/创建、目标查重和分享转存能力。
- 新增只读、单项转存和建目录三个独立探针；凭证只在当前进程使用，不写入数据库、
  文件或日志。
- 账号页面新增 123 云盘类型，Access Token 继续使用现有加密凭证存储边界。

### 可靠性与安全边界

- 分享 URL 仅接受已知官方 HTTPS 格式，请求目标固定为受信任的 123 云盘主机，
  不使用用户输入构造任意请求源。
- 写入操作不做 HTTP 自动重放；超时、断线、服务端异常或无效响应统一进入结果
  不确定状态，避免重复转存或重复建目录。
- 转存使用可恢复操作标识，进程重启后先对账目标目录，再决定任务状态，不盲目
  再次提交远端写入。

### 验证

- 真实账号只读探针通过：账号会话、根目录和分享第一页契约均已确认。
- 真实跨账号单项转存通过，且目标结果已核验；真实根目录建目录探针通过。
- 完整后端测试 699 项、前端测试 5 项、Ruff 与前端生产构建通过。
- 公开 rc.26 双架构镜像和 NAS 页面端到端验收仍需在发布后完成。

## [0.2.0-rc.25] - 2026-08-26

这是失败文件批量恢复候选版本。rc.24 现场升级和夸克单项重试已经成功；后续批量
转存因目标账号空间不足失败，腾出空间后单项重新尝试恢复正常，但逐项操作成本过高。

### 新增

- 文件记录页新增“批量重试失败文件”，按当前订阅和搜索词统计范围并在提交前确认
  数量。
- 服务端批量入口只选择失败文件，自动跳过已有活动任务，单次最多接收 1000 个，
  超限时要求缩小筛选范围。
- 文件搜索同时匹配文件名和分享内相对路径。

### 验证

- 完整后端测试 668 项、前端测试 5 项与 Ruff 检查通过。
- 功能 PR #156 的后端与迁移、前端生产构建、单镜像与部署契约全部通过。
- 公开 rc.25 镜像发布后仍需现场验证批量确认、入队数量和后续转存结果。
