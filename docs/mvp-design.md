...
> 第一阶段仅支持阿里云盘。本方案以四张核心业务表为骨架，同时保留 Provider 扩展、凭证安全、增量检测、幂等转存、失败重试和任务追踪能力。
...
| 阶段 | 主要交付 | 预计工时 |
|---|---|---:|
| 0. 阿里云盘技术验证 | 登录刷新、分享解析、分页遍历、目录创建、转存、限流验证 | 2-4 人日 |
...
如果阿里云盘只能依赖非官方协议或复杂登录流程，额外预留 5-10 人日及持续维护成本。阶段 0 是开发门槛，应先确认账号、分享和转存链路可稳定调用。
